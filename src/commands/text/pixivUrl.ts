    ...
    if (illust) {
      ...
      if (meta_pages.length) {
        attachPromises = meta_pages.map(({ image_urls: image_urls_pages }) => {
          const attach = newAttach([image_urls_pages.original, image_urls_pages.large])
          return attach
        })
      } else {
        attachPromises = await [newAttach([meta_single_page.original_image_url, image_urls.large])]
      }
